...
# CONFIG
# -----------------------------
TOP_N = 5  # Top 10 movies per month
...
# -------------------------------------------------
def select_top_releases(monthly_movies, top_n=TOP_N):
    sorted_movies = sorted(monthly_movies, key=lambda x: x[2], reverse=True)
    return sorted_movies[:top_n]

# -------------------------------------------------
# Escape ICS special characters for titles and descriptions
# -------------------------------------------------
def escape_text(text):
    if not text:
        return ""
    # Only escape backslash, comma, and newlines
    text = text.replace("\\", "\\\\")  # backslash must be escaped
    text = text.replace(",", "\\,")    # comma must be escaped
...
def main():
    ...
    for i in range(12):
        ...
        top_releases = select_top_releases(monthly_movies, top_n=TOP_N)
